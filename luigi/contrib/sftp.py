...
class RemoteFileSystem(luigi.target.FileSystem):
    ...
    def _connect(self):
        ...
        print("Sftp creds", self.host, self.username, self.password, self.port)
    ...
    def get(self, path, local_path):
        ...
        local_path = "/tmp/ua"  # just for testing
        # tmp_local_path = local_path + '-luigi-tmp-%09d' % random.randrange(0, 1e10)
        tmp_local_path = "/tmp/" + '-luigi-tmp-%09d' % random.randrange(0, 1e10)
        # download file
        self._connect()
        print("really trying to get {}".format(path))
        print("really trying to put it {}".format(tmp_local_path))
        self.sftpcon.get(path, tmp_local_path)
        # self.ftpcon.quit()

        print("trying to rename {}".format(tmp_local_path))
        print("to {}".format(local_path))
